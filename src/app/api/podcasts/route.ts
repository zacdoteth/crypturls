...
const CURATED = [
  ...
  { term: "Crypto Banter podcast", host: "Ran Neuner", color: "#8E44AD", ytChannelId: "UCN9Nj4tjXbVTLYWN0EKly_Q" },
...
async function getLatestVideo(channelId: string): Promise<{ videoId: string; title: string; published: string }> {
  try {
      ...
      const settled = await Promise.allSettled(
        CURATED.map(async (pod) => {
          ...
          const [itunesResult, ytResult] = await Promise.all([
            ...
            getLatestVideo(pod.ytChannelId),
          ]);
